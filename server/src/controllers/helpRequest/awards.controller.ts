...
export const getAwardsByRole = async (
  ...
  const cacheKey = `cache:allAwards`;
  ...
  try {
    ...
    if (cached) {
      ...
      res.json({
        success: true,
        message: "Found Awards from REDIS",
    ...
    if (!awards) {
      res
        .status(404)
        .json({ success: false, message: "No awards found for this role." });
      return;
    }

    await redis.set(cacheKey, JSON.stringify(awards), "EX", 300);

    res
      .status(200)
      .json({ success: true, messages: "Found Awards List", data: awards });
  } catch (error) {
...
export const getAwardDetailsById = async (
  req: Request,
  res: Response
): Promise<void> => {
...
export const changeAwardStatus = async (
  req: Request,
  res: Response
): Promise<void> => {
  ...
  try {
    ...
    redis.del(`cache:allAwards`);
    res
      .status(200)
      .json({ success: true, message: "Award status updated successfully." });
  } catch (error) {
